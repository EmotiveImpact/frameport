...
function mark() { return '<svg viewBox="0 0 40 40" width="34" height="34" aria-hidden="true"><rect width="40" height="40" rx="11" fill="currentColor"/><path d="M11 10h18v6H17v5h9v6h-9v5h-6V10Z" fill="white"/><path d="m26 22 7 5-7 5v-10Z" fill="#e7ac85"/></svg>'; }
...
function framerMark() { return '<svg width="25" height="32" viewBox="0 0 24 32" fill="currentColor" aria-hidden="true"><path d="M0 0h24v11H12l12 10H12v11L0 21V11h12L0 0Z"/></svg>'; }
const state = { view: 'overview', tab: 'preview', jobs: offline ? [offline.job] : [], job: null, worker: false, auth: false, key: '', width: 1440, page: 'p0', mode: 'compare', split: 50, files: [], code: '', file: '', contents: [], edits: new Map(), historyFilter: '', sourceTarget: 'react', poll: 0, formTab: 'url', zoom: false };
let toastTimer = 0;
...
async function api(path, options = {}) {
    ...
    const response = await fetch(path, { ...options, headers: { 'Content-Type': 'application/json', ...(state.key ? { 'Authorization': `Bearer ${state.key}` } : {}), ...options.headers } });
    if (!response.ok) {
...
async function boot() { render(); if (offline) {
    state.worker = false;
    return;
} try {
    const health = await api('/api/health');
    state.worker = health.worker === 'available';
    state.auth = health.authRequired;
...
function shell(content) { const active = state.view === 'project' ? 'history' : state.view; return `<aside class="sidebar"><a href="#" class="brand" data-view="overview">${mark()}<span>frameport<span class="brand-dot">.</span></span></a><button class="workspace" id="workspace-button"><span class="avatar small">E</span><span>Emotive workspace<small>Personal workspace</small></span>${icon('down', 14)}</button><div class="nav-label">WORKSPACE</div><nav class="main-nav"><button class="${active === 'overview' ? 'active' : ''}" data-view="overview">${icon('grid')}Overview</button><button class="${active === 'history' ? 'active' : ''}" data-view="history">${icon('layers')}Conversions<span class="nav-count">${state.jobs.length}</span></button><button class="${active === 'docs' ? 'active' : ''}" data-view="docs">${icon('book')}Documentation${icon('up', 13)}</button></nav><div class="sidebar-spacer"></div><div class="sidebar-note"><div class="tiny-glyph">${icon('code', 22)}</div><strong>A new home for<br>your best work.</strong><p>Take your design further.<br>Keep the possibilities open.</p><span class="build-label">DEVELOPER RELEASE <i>0.1</i></span></div><div class="worker-status"><span class="dot ${state.worker ? 'online' : ''}"></span><span>${offline ? 'Portable preview' : state.worker ? 'Local worker connected' : 'Worker not connected'}</span>${icon('info', 13)}</div><button class="profile" id="profile-button"><span class="avatar">E</span><span>Emotive Impact<small>${offline ? 'Preview workspace' : 'Private workspace'}</small></span>${icon('sliders', 16)}</button></aside><main class="main"><header class="topbar"><div class="breadcrumb"><button data-view="overview">Workspace</button>${icon('chevron', 12)}<span>${state.view === 'project' ? esc(title(state.job)) : state.view === 'docs' ? 'Documentation' : state.view === 'history' ? 'Conversions' : 'Overview'}</span></div><div class="top-actions"><button class="search-shortcut" id="find-button">${icon('search', 15)}<span>Find a conversion</span><kbd>⌘ K</kbd></button><span class="top-divider"></span><button class="icon-button" data-view="docs" aria-label="Help and documentation">${icon('book', 17)}</button><span class="avatar top-avatar">E</span></div></header><div class="page ${state.view === 'project' ? 'project-page' : ''}">${content}</div><footer class="app-footer"><span>Made for what comes next.</span><span>Frameport 0.1 <b>·</b> ${offline ? 'Interactive preview, not a live worker' : 'Single-workspace developer build'}</span></footer></main><div id="modal-root"></div>`; }
function overview() { return `<section class="welcome"><div><div class="eyebrow"><span class="orange-line"></span> GOOD DESIGN. MORE POSSIBILITIES.</div><h1>Your design.<br>Your code. <span>Your rules.</span></h1><p>Bring your Framer website into your own codebase.<br>Editable React and HTML. A new beginning, not a rebuild.</p></div><div class="transfer-art" aria-hidden="true"><div class="art-orbit orbit-one"></div><div class="art-orbit orbit-two"></div><span class="art-star star-one">+</span><span class="art-star star-two">+</span><div class="source-tile">${framerMark()}<span>Framer</span></div><div class="transfer-line"><span></span>${icon('arrow', 22)}</div><div class="target-tile"><div class="target-icons"><span>${reactMark(27)}</span><span>${icon('code', 26)}</span></div><strong>Yours to build on.</strong><small>REACT + HTML</small></div><div class="floating-tag">${icon('check', 12)} No Framer runtime</div></div></section><section class="convert-card"><div class="convert-card-head"><div class="input-tabs"><button id="url-tab" class="${state.formTab === 'url' ? 'selected' : ''}">${icon('globe', 15)}Website URL</button><button id="bridge-tab" class="${state.formTab === 'bridge' ? 'selected' : ''}">${icon('layers', 15)}Project manifest<span class="mini-label">BRIDGE</span></button></div><span class="privacy-note">${icon('lock', 13)} Private to your workspace</span></div>${state.formTab === 'url' ? `<form id="conversion-form"><label class="input-label" for="url">Where’s your next project coming from?</label><div class="url-input-wrap">${icon('globe', 20)}<input id="url" type="url" placeholder="https://your-website.framer.website" autocomplete="url" spellcheck="false" required><button class="primary convert-button" type="submit">Convert website ${icon('arrow', 17)}</button></div><div class="input-bottom"><span>${icon('info', 13)}Use the published website, not the Framer editor link.</span><button type="button" class="text-button" id="options-toggle">${icon('sliders', 14)}Export settings</button></div><div class="export-options" id="export-options" hidden><label>Project name<input id="project-name" placeholder="My next project" maxlength="80"></label><label>Page limit<select id="max-pages"><option value="1">1 page</option><option value="3" selected>Up to 3 pages</option><option value="8">Up to 8 pages</option></select></label><label class="check-option"><input id="crawl" type="checkbox" checked>Follow same-site links</label></div><label class="permission"><input id="permission" type="checkbox" required><span>I own this website or have permission to export its design and assets.</span></label></form>` : `<div class="bridge-panel"><span class="bridge-icon">${icon('upload', 26)}</span><div><h3>Bring the project context with you.</h3><p>Import a Frameport manifest with a published URL, project details and optional CMS content. The published site is still required for capture.</p><label class="secondary upload-label">Choose manifest.json<input id="manifest-input" type="file" accept="application/json,.json" hidden></label><button class="text-button" id="bridge-help">Read the bridge guide ${icon('up', 13)}</button></div></div>`}<div class="output-strip"><span>YOUR EXPORT INCLUDES</span><div>${reactMark(17)} React + TypeScript</div><div>${icon('code', 16)} HTML + CSS</div><div>${icon('shield', 16)} Visual evidence</div><span class="no-keys">No AI API key needed</span></div></section><section class="below-converter"><div class="mini-feature">${icon('layers', 20)}<div><strong>More than a flat copy</strong><p>Separate components, pages and content.</p></div></div><div class="mini-feature">${icon('monitor', 20)}<div><strong>Responsive, with receipts</strong><p>Compare five widths against the original.</p></div></div><div class="mini-feature">${icon('code', 20)}<div><strong>Keep building, anywhere</strong><p>Your files. Your editor. Your next move.</p></div></div></section><section class="recent-section"><div class="section-title"><h2>${state.jobs.length ? 'Recent conversions' : 'Your next chapter starts here'}</h2>${state.jobs.length ? '<button class="text-button" data-view="history">View all ' + icon('arrow', 14) + '</button>' : '<span class="subtle">A good place to try things out.</span>'}</div>${state.jobs.length ? `<div class="recent-grid">${state.jobs.slice(0, 3).map(jobCard).join('')}</div>` : `<div class="first-project"><div class="sample-art"><span class="sample-logo">forma<span>®</span></span><div class="sample-shape"></div><span class="sample-caption">INDEPENDENT BY DESIGN.</span></div><div class="sample-copy"><span class="eyebrow">INCLUDED TEST WEBSITE</span><h3>Meet your first conversion.</h3><p>Try the full workflow with Forma, our original two-page test site.<br>Explore the source, check the differences and make it yours.</p><button class="secondary" id="run-demo">${icon('play', 14)}${offline ? 'Explore the tested export' : 'Convert the sample site'} ${icon('arrow', 14)}</button></div><span class="sample-corner">LOCAL WORKER.<br>REAL OUTPUT.</span></div>`}</section><div class="scope-note">${icon('info', 15)}<span>Evidence, not promises. Visual checks cover the captured HTML states. Framer-specific interactions and the React production build need separate review.</span></div>`; }
...
function render() { const app = $('#app'); if (!app)
    return; app.innerHTML = shell(state.view === 'overview' ? overview() : state.view === 'history' ? history() : state.view === 'docs' ? docs() : project()); bind(); }
async function loadJob(id) { try {
    state.job = await api(`/api/jobs/${id}`);
    state.view = 'project';
    state.page = state.job.report?.pages[0]?.key || 'p0';
    state.tab = 'preview';
    state.edits.clear();
    state.files = [];
    state.code = '';
...
function pollJob() { clearTimeout(state.poll); if (!state.job || !['queued', 'running'].includes(state.job.status))
    return; state.poll = window.setTimeout(async () => { try {
...
async function submitJob(payload) { try {
...
async function changeTab(tab) { state.tab = tab; render(); try {
    if (tab === 'source') {
        state.files = await api(`/api/jobs/${state.job.id}/files?target=${state.sourceTarget}`);
        if (!state.files.find(f => f.path === state.file))
            state.file = state.files.find(f => f.path.endsWith('Hero.tsx'))?.path || state.files.find(f => f.path.endsWith('App.tsx'))?.path || state.files[0]?.path || '';
        if (state.file)
    ...
    if (tab === 'content') {
        state.contents = await api(`/api/jobs/${state.job.id}/content`);
        render();
    }
}
catch (e) {
...
async function loadFile(path) { try {
    const file = await api(`/api/jobs/${state.job.id}/file?target=${state.sourceTarget}&path=${encodeURIComponent(path)}`);
    state.file = path;
...
function showAccess() { const root = $('#modal-root'); if (!root)
    return; root.innerHTML = `<div class="modal-backdrop"><section class="modal" role="dialog" aria-modal="true" aria-labelledby="access-title"><button class="icon-button modal-close" id="close-modal" aria-label="Close">${icon('close', 18)}</button><span class="modal-icon">${icon('lock', 26)}</span><h2 id="access-title">Your workspace.<br>Your access.</h2><p>${offline ? 'This is a self-contained preview of the real application. Run the included worker for live conversions.' : 'Enter the FRAMEPORT_API_KEY configured on your worker. It is kept in memory for this tab, not browser storage.'}</p>${offline ? '<button class="primary" id="modal-docs">Read setup instructions</button>' : '<form id="access-form"><label>Worker access key<input type="password" id="access-key" autocomplete="off" required></label><button class="primary" type="submit">Connect workspace ' + icon('arrow', 15) + '</button></form>'}</section></div>`; $('#close-modal')?.addEventListener('click', () => root.innerHTML = ''); $('#modal-docs')?.addEventListener('click', () => { state.view = 'docs'; render(); }); $('#access-key')?.focus(); $('#access-form')?.addEventListener('submit', async (e) => { e.preventDefault(); state.key = $('#access-key').value; try {
    state.jobs = await api('/api/jobs');
    state.auth = false;
    render();
...
function bind() {
    $$('[data-view]').forEach(el => el.addEventListener('click', e => { e.preventDefault(); state.view = el.getAttribute('data-view'); render(); window.scrollTo(0, 0); }));
    $$('[data-job]').forEach(el => el.addEventListener('click', () => loadJob(el.getAttribute('data-job'))));
    $('#run-demo')?.addEventListener('click', runDemo);
    $('#find-button')?.addEventListener('click', () => { state.view = 'history'; render(); $('#history-search')?.focus(); });
    ...
    $('#bridge-help')?.addEventListener('click', () => { state.view = 'docs'; render(); });
    ...
    $$('[data-width]').forEach(el => el.addEventListener('click', () => { state.width = Number(el.getAttribute('data-width')); render(); }));
    $('#page-select')?.addEventListener('change', e => { state.page = e.target.value; render(); });
    ...
    $('#save-content')?.addEventListener('click', async () => { try {
        state.job = await api(`/api/jobs/${state.job.id}/edits`, { method: 'POST', body: JSON.stringify({ patches: Array.from(state.edits).map(([id, text]) => ({ id, text })) }) });
        state.edits.clear();
        render();
        pollJob();
    }
    ...
    $$('[data-inspect]').forEach(el => el.addEventListener('click', () => { const [p, w] = el.getAttribute('data-inspect').split(':'); state.page = p; state.width = Number(w); state.mode = 'diff'; state.tab = 'preview'; render(); }));
}
