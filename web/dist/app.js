...
function render() { eclipse?.dispose(); eclipse = null; const app = $('#app'); if (!app)
    return; app.innerHTML = shell(state.view === 'overview' ? overview() : state.view === 'history' ? history() : state.view === 'docs' ? docs() : project()); bind(); const canvas = $('#hero-shader'); if (canvas) {
