...
import { connectionBanner, settingsView, loginView, problemView, exportView, type Health } from './platform-views.js';
import { mountEclipse, type EclipseController } from './eclipse.js';
/* Frameport Studio. No UI frameworks or remote dependencies are required at runtime. */
type Json = Record<string, unknown>;
interface Comparison {page:string;route:string;width:number;source:string;export:string;diff:string;passed:boolean;matchedPixels:number;difference:number;changedPixels:number;totalPixels:number;capturedHeight:number;documentHeight:number}
interface Issue {code:string;severity:string;message:string;page?:string;count?:number}
interface Report {execution?:{mode:string;description:string;browserNavigationVerified:boolean};verdict:string;visualPassed:boolean;checksPassed:boolean;comparisons:Comparison[];checks:{name:string;passed:boolean;page:string;width?:number}[];issues:Issue[];components:{name:string;file:string;page:string}[];assets:number;nodes:number;textCount:number;pages:{key:string;route:string;title:string;framerDetected:boolean}[];coverage:{widths:number[];maxHeight:number;target:string};reactBuild:{status:string;message:string};revision:number;demo:boolean;caveat:string}
interface Job {id:string;created:number;updated:number;status:string;stage:string;progress:number;error:string|null;request:{url:string;name:string;demo:boolean;max_pages:number};report:Report|null;events:{time:number;stage:string;message:string}[];revision:number;ticket?:string}
interface PreviewData {job:Job;evidence:Record<string,string>;files:Record<string,string>;htmlFiles:Record<string,string>;content:{id:string;text:string}[];downloads:Record<string,string>}
declare global {interface Window {__FRAMEPORT_PREVIEW__?:PreviewData}}
...
const $$ = <T extends Element = HTMLElement>(selector:string):T[] => Array.from(document.querySelectorAll<T>(selector));
...
function icon(name:string,size=18):string {return `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.55" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${paths[name]||paths.file}</svg>`;}
function mark():string {return '<svg viewBox="0 0 32 32" width="25" height="25" aria-hidden="true"><path d="M4 4h24v8H12v5h12v7H12v4H4V4Z" fill="currentColor"/><path d="m21 17 7 5-7 6V17Z" fill="currentColor"/></svg>';}
function reactMark(size=23):string{return `<svg width="${size}" height="${size}" viewBox="0 0 32 32" fill="none" stroke="currentColor" stroke-width="1.3" aria-hidden="true"><ellipse cx="16" cy="16" rx="14" ry="5.5"/><ellipse cx="16" cy="16" rx="14" ry="5.5" transform="rotate(60 16 16)"/><ellipse cx="16" cy="16" rx="14" ry="5.5" transform="rotate(120 16 16)"/><circle cx="16" cy="16" r="2.2" fill="currentColor" stroke="none"/></svg>`;}
const state = {view:'overview',tab:'preview',jobs:offline?[offline.job]:[] as Job[],job:null as Job|null,worker:false,auth:false,key:'',width:1440,page:'p0',mode:'compare',split:50,files:[] as {path:string;bytes:number}[],code:'',file:'',contents:[] as {id:string;text:string}[],edits:new Map<string,string>(),historyFilter:'',sourceTarget:'react',poll:0,formTab:'url',zoom:false,loading:false,problem:'',problemKind:'offline',filterStatus:'all',health:{worker:'checking',canConvert:false,authRequired:false,storage:'not-connected'} as Health};
let toastTimer = 0;
function toast(message:string){const el=$('#toast');if(!el)return;el.textContent=message;el.classList.add('visible');clearTimeout(toastTimer);toastTimer=window.setTimeout(()=>el.classList.remove('visible'),4300);}
async function api<T>(path:string,options:RequestInit={}):Promise<T>{
  if(offline){
    if(options.method && options.method!=='GET')throw new Error('This is the portable preview. Start the included Frameport worker to run conversions and save edits.');
    if(path==='/api/jobs'||path==='/api/projects')return [offline.job] as T;
    if(path==='/api/session')return {authenticated:true,authRequired:false} as T;
    if(path==='/api/health')return {worker:'preview',authRequired:false} as T;
    if(path.includes('/files')){const files=path.includes('target=html')?offline.htmlFiles:offline.files;return Object.keys(files).map(p=>({path:p,bytes:files[p].length})) as T;}
    if(path.includes('/file?')){const u=new URL(path,'http://frameport.local/');const p=u.searchParams.get('path')||'';return {path:p,content:(u.searchParams.get('target')==='html'?offline.htmlFiles:offline.files)[p]||''} as T;}
    if(path.endsWith('/content'))return offline.content as T;
    return offline.job as T;
  }
...
function jobCard(job:Job){let visual='';if(job.status==='completed'&&job.report){const c=job.report.comparisons.find(c=>c.page==='p0'&&c.width===1440);if(c)visual=`<img src="${evidenceURL(job,c.export)}" alt="Actual exported page preview" loading="lazy">`;}
...
let eclipse: EclipseController | null = null;
let motionPaused = false;
function render(){
  eclipse?.dispose();eclipse=null;
  const app=$('#app');if(!app)return;
  let content:string;
  if(state.loading)content='<div class="screen-loading" role="status"><span class="busy-marker"></span><h1>Opening your workspace.</h1><p>Loading the saved project and its current revision.</p></div>';
  else if(state.problem)content=problemView(state.problem,state.problemKind);
  else content=state.view==='overview'?overview():state.view==='convert'?convertView():state.view==='history'?history():state.view==='docs'?docs():state.view==='settings'?settingsView(state.health,!state.auth&&backendConnected()):state.view==='login'?loginView('',backendConnected()):state.view==='project'&&state.job?project():problemView('This address does not match a page in Frameport.','notfound');
  app.innerHTML=shell(content);bind();
  const canvas=$<HTMLCanvasElement>('#hero-shader');
  if(canvas){eclipse=mountEclipse(canvas,{paused:motionPaused,onState:({motion})=>{const button=$<HTMLButtonElement>('#motion-toggle');if(!button)return;button.disabled=motion==='unavailable'||motion==='reduced';button.textContent=motion==='unavailable'?'Static artwork':motion==='reduced'?'Reduced motion':motion==='paused'?'Play motion':'Pause motion';button.setAttribute('aria-pressed',String(motion==='paused'));button.setAttribute('aria-label',motion==='paused'?'Play background animation':'Pause background animation');}});}
}
window.addEventListener('pagehide',()=>{eclipse?.dispose();eclipse=null;});
window.addEventListener('pageshow',event=>{if(event.persisted)render();});
async function loadJob(id:string){await navigate(projectPath(id));}
function pollJob(){
  clearTimeout(state.poll);
  if(!state.job||!['queued','running'].includes(state.job.status)||state.view!=='project')return;
  const id=state.job.id, generation=routeGeneration;
  state.poll=window.setTimeout(async()=>{
    try {
      const job=await api<Job>(`/api/jobs/${id}`);
      if(generation!==routeGeneration||state.job?.id!==id)return;
      state.job=job;upsertJob(job);
      if(job.status==='completed'){state.files=[];state.code='';state.contents=[];render();if(['source','content'].includes(state.tab))await changeTab(state.tab,false);}
      else render();
      pollJob();
    }catch(e){if(generation===routeGeneration){toast((e as Error).message);state.poll=window.setTimeout(pollJob,3000);}}
  },document.hidden?3000:1100);
}
async function submitJob(payload:Json){
  if(submitting)return;
  if(!offline&&(!state.worker||state.auth)){await navigate(state.auth?'/login':'/settings');return;}
  submitting=true;
  const button=$<HTMLButtonElement>('.convert-button');if(button)button.disabled=true;
  try{const job=await api<Job>('/api/jobs',{method:'POST',headers:{'Idempotency-Key':crypto.randomUUID()},body:JSON.stringify(payload)});upsertJob(job);state.edits.clear();await navigate(projectPath(job.id));}
  catch(e){toast((e as Error).message);if(button)button.disabled=false;}
  finally{submitting=false;}
}
async function runDemo(){if(offline){await loadJob(offline.job.id);return;}await submitJob({demo:true,permission:true,name:'Forma Studio',max_pages:3,crawl:true,format:'both'});}
async function changeTab(tab:string,push=true){
  if(!state.job)return;
  const id=state.job.id, revision=state.job.revision;
  state.tab=tab;
  if(push)writeLocation(projectPath(id,tab));
  const generation=routeGeneration;
  render();
  try{
    if(tab==='source'){
      const target=state.sourceTarget;
      const files=await api<{path:string;bytes:number}[]>(`/api/jobs/${id}/files?target=${target}`);
      if(generation!==routeGeneration||state.tab!==tab||state.job?.id!==id||state.job.revision!==revision||state.sourceTarget!==target)return;
      state.files=files;
      if(!files.some(f=>f.path===state.file))state.file=files.find(f=>f.path.endsWith('Hero.tsx'))?.path||files.find(f=>f.path.endsWith('App.tsx'))?.path||files[0]?.path||'';
      if(state.file)await loadFile(state.file);else render();
    }
    if(tab==='content'){
      const contents=await api<{id:string;text:string}[]>(`/api/jobs/${id}/content`);
      if(generation!==routeGeneration||state.tab!==tab||state.job?.id!==id||state.job.revision!==revision)return;
      state.contents=contents;render();
    }
  }catch(e){if(generation===routeGeneration)toast((e as Error).message);}
}
async function loadFile(path:string){
  if(!state.job)return;
  const request=++fileGeneration,id=state.job.id,revision=state.job.revision,target=state.sourceTarget;
  try{
    const file=await api<{content:string}>(`/api/jobs/${id}/file?target=${target}&path=${encodeURIComponent(path)}`);
    if(request!==fileGeneration||state.job?.id!==id||state.job.revision!==revision||state.tab!=='source'||state.sourceTarget!==target)return;
    state.file=path;state.code=file.content;syncQuery();render();
  }catch(e){if(request===fileGeneration)toast((e as Error).message);}
}
async function download(target:string){try{let url:string;let name:string;if(offline){url=target==='report'?'data:application/json;charset=utf-8,'+encodeURIComponent(JSON.stringify(offline.job.report,null,2)):offline.downloads[target];name=target==='report'?'frameport-verification.json':`frameport-forma-${target}.zip`;}else if(target!=='report'){const links=await api<Record<string,{url:string}>>(`/api/jobs/${state.job!.id}/export-links`);const a=document.createElement('a');a.href=links[target].url;a.rel='noreferrer';a.download=`frameport-${target}.zip`;a.click();toast('Your download is starting.');return;}else{const path=target==='report'?`/api/jobs/${state.job!.id}/report`:`/api/jobs/${state.job!.id}/download/${target}`;const response=await fetch(path,{headers:state.key?{Authorization:`Bearer ${state.key}`}:{}});if(!response.ok)throw new Error('Export is unavailable. Reload this conversion.');url=URL.createObjectURL(await response.blob());name=target==='report'?'frameport-verification.json':`frameport-${target}-${state.job!.id.slice(0,8)}.zip`;}
const a=document.createElement('a');a.href=url;a.download=name;a.click();if(url.startsWith('blob:'))setTimeout(()=>URL.revokeObjectURL(url),60000);toast('Your export is ready.');}catch(e){toast((e as Error).message);}}
function showAccess(){if(!offline&&!backendConnected()){void navigate('/settings');return;}const root=$('#modal-root');if(!root)return;root.innerHTML=`<div class="modal-backdrop"><section class="modal" role="dialog" aria-modal="true" aria-labelledby="access-title"><button class="icon-button modal-close" id="close-modal" aria-label="Close">${icon('close',18)}</button><span class="modal-icon">${icon('lock',26)}</span><h2 id="access-title">Your workspace.<br>Your access.</h2><p>${offline?'This is a self-contained preview of the real application. Run the included worker for live conversions.':'Enter the FRAMEPORT_API_KEY configured on your worker. It is exchanged for an HTTP-only session. Your sign-in survives refreshes.'}</p>${offline?'<button class="primary" id="modal-docs">Read setup instructions</button>':'<form id="access-form"><label>Worker access key<input type="password" id="access-key" autocomplete="off" required></label><button class="primary" type="submit">Connect workspace '+icon('arrow',15)+'</button></form>'}</section></div>`;$('#close-modal')?.addEventListener('click',()=>root.innerHTML='');$('#modal-docs')?.addEventListener('click',()=>{void navigate('/docs');});$('#access-key')?.focus();$('#access-form')?.addEventListener('submit',async e=>{e.preventDefault();const key=$<HTMLInputElement>('#access-key')!.value;try{await api('/api/session',{method:'POST',body:JSON.stringify({key})});state.key='';state.auth=false;await refreshHealth();await applyRoute(activePath);toast('Workspace connected.');}catch(e){toast((e as Error).message);}});}
function bind(){
  bindPlatform();
  $('#start-converting')?.addEventListener('click',()=>{$('#convert')?.scrollIntoView({behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth'});$<HTMLInputElement>('#url')?.focus({preventScroll:true});});
  ...
  $('#workspace-button')?.addEventListener('click',showAccess);$('#profile-button')?.addEventListener('click',showAccess);
  $('#url-tab')?.addEventListener('click',()=>{state.formTab='url';render();});$('#bridge-tab')?.addEventListener('click',()=>{state.formTab='bridge';render();});$('#bridge-help')?.addEventListener('click',()=>{void navigate('/docs');});
  $('#options-toggle')?.addEventListener('click',()=>{const el=$('#export-options');if(el){el.hidden=!el.hidden;$('#options-toggle')?.setAttribute('aria-expanded',String(!el.hidden));}});
  $('#conversion-form')?.addEventListener('submit',async e=>{e.preventDefault();await submitJob({url:$<HTMLInputElement>('#url')!.value,name:$<HTMLInputElement>('#project-name')?.value||'',permission:$<HTMLInputElement>('#permission')!.checked,max_pages:Number($<HTMLSelectElement>('#max-pages')?.value||3),crawl:$<HTMLInputElement>('#crawl')?.checked??true,format:'both'});});
  $('#manifest-input')?.addEventListener('change',async e=>{try{const file=(e.target as HTMLInputElement).files?.[0];if(!file)return;if(file.size>1_000_000)throw new Error('Manifest must be smaller than 1 MB.');const manifest=JSON.parse(await file.text());if(manifest.schema!=='frameport.bridge.v1'||!manifest.publishedUrl)throw new Error('Choose a valid Frameport bridge manifest with a published URL.');if(!confirm('Confirm that you own this project or have permission to export its design and included content.'))return;await submitJob({url:manifest.publishedUrl,name:manifest.project?.name||'',permission:true,max_pages:3,crawl:true,format:'both',bridge:manifest});}catch(e){toast((e as Error).message);}});
  $('#history-search')?.addEventListener('input',e=>{const input=e.target as HTMLInputElement;const pos=input.selectionStart;state.historyFilter=input.value;render();const next=$<HTMLInputElement>('#history-search');next?.focus();next?.setSelectionRange(pos,pos);});
  ...
  $('#page-select')?.addEventListener('change',e=>{state.page=(e.target as HTMLSelectElement).value;syncQuery();render();});
  $('#preview-mode')?.addEventListener('change',e=>{state.mode=(e.target as HTMLSelectElement).value;render();});
  $('#compare-range')?.addEventListener('input',e=>{state.split=Number((e.target as HTMLInputElement).value);const im=$<HTMLElement>('.export-image');const ln=$<HTMLElement>('.compare-line');if(im)im.style.clipPath=`inset(0 0 0 ${state.split}%)`;if(ln)ln.style.left=`${state.split}%`;});
  $('#zoom-button')?.addEventListener('click',()=>{state.zoom=!state.zoom;render();});
  $('#download-react')?.addEventListener('click',()=>download('react'));$('#download-html')?.addEventListener('click',()=>download('html'));$('#download-report')?.addEventListener('click',()=>download('report'));
  $('#cancel-job')?.addEventListener('click',async()=>{try{state.job=await api(`/api/jobs/${state.job!.id}/cancel`,{method:'POST',body:'{}'});render();pollJob();}catch(e){toast((e as Error).message);}});
  $('#retry-job')?.addEventListener('click',async()=>{try{const j=await api<Job>(`/api/jobs/${state.job!.id}/retry`,{method:'POST',body:'{}'});upsertJob(j);await navigate(projectPath(j.id));}catch(e){toast((e as Error).message);}});
  $$('[data-file]').forEach(el=>el.addEventListener('click',()=>loadFile(el.getAttribute('data-file')!)));
  $('#source-target')?.addEventListener('change',e=>{state.sourceTarget=(e.target as HTMLSelectElement).value;state.file='';changeTab('source');});
  $('#copy-code')?.addEventListener('click',async()=>{try{await navigator.clipboard.writeText(state.code);toast('Source code copied.');}catch{toast('Clipboard access is unavailable. Select and copy the source directly.');}});
  $$<HTMLTextAreaElement>('[data-content]').forEach(el=>el.addEventListener('input',()=>{const id=el.dataset.content!;const original=state.contents.find(c=>c.id===id)?.text;if(el.value===original)state.edits.delete(id);else state.edits.set(id,el.value);const btn=$<HTMLButtonElement>('#save-content');if(btn){btn.disabled=!state.edits.size;btn.innerHTML=icon('refresh',15)+`Rebuild${state.edits.size?' '+state.edits.size+' edits':''}`;}}));
  $('#save-content')?.addEventListener('click',async()=>{const button=$<HTMLButtonElement>('#save-content');if(button)button.disabled=true;try{state.job=await api(`/api/jobs/${state.job!.id}/edits`,{method:'POST',body:JSON.stringify({expected_revision:state.job!.revision,patches:Array.from(state.edits).map(([id,text])=>({id,text}))})});state.edits.clear();render();pollJob();}catch(e){toast((e as Error).message);if(button)button.disabled=false;}});
...
function upsertJob(job:Job){const index=state.jobs.findIndex(j=>j.id===job.id);if(index<0)state.jobs.unshift(job);else state.jobs[index]=job;}
async function refreshHealth(){
  if(offline)return;
  try{
    state.health=await api<Health>('/api/health');state.worker=state.health.canConvert===true;
    state.auth=state.health.authRequired;
    if(backendConnected()){
      const session=await api<{authenticated:boolean}>('/api/session');state.auth=!session.authenticated;
      if(!state.auth)state.jobs=await api<Job[]>('/api/jobs');
    }
  }catch{state.worker=false;state.health={worker:'offline',canConvert:false,authRequired:false,storage:'not-connected',message:'The workspace service is not responding.'};}
}
function writeLocation(path:string,replace=false){
  activePath=path;
  if(!offline&&/^https?:$/.test(location.protocol))window.history[replace?'replaceState':'pushState']({},'',path);
}
function syncQuery(){
  if(!state.job||state.view!=='project'||offline)return;
  const url=new URL(projectPath(state.job.id,state.tab),location.href);
  if(state.page!=='p0')url.searchParams.set('page',state.page);
  if(state.width!==1440)url.searchParams.set('width',String(state.width));
  if(state.tab==='source'&&state.file)url.searchParams.set('file',state.file);
  if(state.sourceTarget!=='react')url.searchParams.set('target',state.sourceTarget);
  writeLocation(url.pathname+url.search,true);
}
async function navigate(path:string,replace=false){
  const route=parseRoute(path);
  if(state.edits.size&&route.id!==state.job?.id&&!confirm('Leave this project and discard unsaved text edits?'))return;
  if(route.id!==state.job?.id)state.edits.clear();
  writeLocation(path,replace);await applyRoute(path);window.scrollTo(0,0);
}
async function applyRoute(path:string){
  const generation=++routeGeneration;++fileGeneration;clearTimeout(state.poll);
  const route=parseRoute(path);activePath=path;
  state.problem='';state.loading=false;state.view=route.view;state.tab=route.tab;
  state.file=route.file;state.sourceTarget=route.target;state.page=route.page;state.width=route.width;
  if(['project','history'].includes(route.view)){
    if(!backendConnected()){
      state.problem='The persistent workspace service is not connected. No temporary or simulated projects are being shown.';state.problemKind='offline';render();return;
    }
    if(state.auth){state.view='login';render();return;}
    state.loading=true;render();
    try{
      if(route.view==='history')state.jobs=await api<Job[]>('/api/jobs');
      else {
        const job=await api<Job>(`/api/jobs/${route.id}`);
        if(generation!==routeGeneration)return;
        state.job=job;upsertJob(job);state.files=[];state.code='';state.contents=[];
        if(job.report&&!job.report.pages.some(p=>p.key===state.page))state.page=job.report.pages[0]?.key||'p0';
      }
    }catch(e){
      if(generation!==routeGeneration)return;
      if(e instanceof ApiError&&e.status===401){state.auth=true;state.view='login';}
      else {state.problem=(e as Error).message;state.problemKind=e instanceof ApiError&&e.status===404?'notfound':'offline';}
    }
  }
  if(generation!==routeGeneration)return;
  state.loading=false;render();
  if(route.view==='project'&&state.job&&!state.problem&&state.view!=='login'){
    if(state.job.status==='completed'&&['source','content'].includes(state.tab))await changeTab(state.tab,false);
    else pollJob();
  }
...
function bindPlatform(){
  $$('[data-route]').forEach(el=>el.addEventListener('click',e=>{if(e instanceof MouseEvent&&(e.metaKey||e.ctrlKey||e.shiftKey||e.button!==0))return;e.preventDefault();void navigate(el.getAttribute('data-route')!);}));
  ...
  $('#logout-workspace')?.addEventListener('click',async()=>{try{await api('/api/session',{method:'DELETE',body:'{}'});state.key='';state.auth=true;state.jobs=[];state.job=null;await navigate('/login');}catch(e){toast((e as Error).message);}});
  $('#platform-login')?.addEventListener('submit',async e=>{
    e.preventDefault();const input=$<HTMLInputElement>('#login-key')!,button=$<HTMLButtonElement>('#platform-login button')!;button.disabled=true;
    try{
      await api('/api/session',{method:'POST',body:JSON.stringify({key:input.value})});input.value='';state.key='';state.auth=false;
      await refreshHealth();const returnPath=activePath.startsWith('/projects/')?activePath:safeReturnPath(new URL(location.href).searchParams.get('next')||'/projects');
      await navigate(returnPath,true);
    }catch(error){const message=$('#platform-login .form-error');if(message)message.textContent=(error as Error).message;button.disabled=false;}
  });
  $('#status-filter')?.addEventListener('change',e=>{state.filterStatus=(e.target as HTMLSelectElement).value;render();});
  $$('[data-export]').forEach(el=>el.addEventListener('click',()=>download(el.getAttribute('data-export')!)));
  $('#copy-project-link')?.addEventListener('click',async()=>{try{await navigator.clipboard.writeText(new URL(projectPath(state.job!.id,state.tab),location.href).href);toast('Project link copied. Sign-in is still required.');}catch{toast('Copy this project’s address from your browser.');}});
  $('#rename-project')?.addEventListener('click',()=>{
    const root=$('#modal-root');if(!root||!state.job)return;
    ...
    $('#rename-input')?.focus();$('#cancel-rename')?.addEventListener('click',()=>root.innerHTML='');
    $('#rename-form')?.addEventListener('submit',async e=>{e.preventDefault();try{const job=await api<Job>(`/api/jobs/${state.job!.id}`,{method:'PATCH',body:JSON.stringify({name:$<HTMLInputElement>('#rename-input')!.value})});state.job=job;upsertJob(job);render();}catch(error){toast((error as Error).message);}});
  });
  $('#delete-project')?.addEventListener('click',async()=>{
    if(!state.job||!confirm('Permanently delete this project, its files and verification evidence?'))return;
    try{await api(`/api/jobs/${state.job.id}`,{method:'DELETE',body:'{}'});state.jobs=state.jobs.filter(j=>j.id!==state.job!.id);state.job=null;state.edits.clear();await navigate('/projects');toast('Project deleted.');}catch(e){toast((e as Error).message);}
  });
  if(!offline&&(!state.worker||state.auth)){
    for(const selector of ['.convert-button','#run-demo']){const button=$<HTMLButtonElement>(selector);if(button){button.disabled=true;button.title=state.auth?'Sign in to convert.':'Connect the browser service in Workspace settings.';}}
  }
}
...
document.addEventListener('keydown',e=>{if((e.metaKey||e.ctrlKey)&&e.key.toLowerCase()==='k'){e.preventDefault();void navigate('/projects').then(()=>$('#history-search')?.focus());}if(e.key==='Escape'){const root=$('#modal-root');if(root)root.innerHTML='';}});
void boot();
