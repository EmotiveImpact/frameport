        ...
        with server(create_app(settings),worker_sock) as worker_origin, server(create_gateway(worker_origin,local_test=True),gateway_sock) as origin:
            async with async_playwright() as pw:
                ...
                try:
                    ...
                    check('Independent visitor cannot inherit the owner session', (await anonymous.request.get(origin+'/api/jobs')).status==401)
                    check('Independent session endpoint remains signed out', (await (await anonymous.request.get(origin+'/api/session')).json())['authenticated'] is False)
